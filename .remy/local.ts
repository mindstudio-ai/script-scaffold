...
function startWatcher(ws: WebSocket) {
  ...
  watcher.on('change', async () => {
    try {
      const code = await fsp.readFile(LOCAL_FILE, 'utf8');
      const h = hash(code);
...
function printBanner(direction: string) {
  console.log();
  console.log(`  ${green('⚡')} ${bold('MindStudio Script Dev')}`);
  console.log();
  console.log(
    `  ${green('➜')}  ${bold('Editing:')}   src/index.ts`,
  );
  console.log(
...
async function main() {
  ...
  const localCode = hasLocalFile
    ? await fsp.readFile(LOCAL_FILE, 'utf8')
    : '';
  ...
  if (localIsEmpty && remoteCode) {
    ...
    await fsp.writeFile(LOCAL_FILE, remoteCode, 'utf8');
    lastHash = hash(remoteCode);
